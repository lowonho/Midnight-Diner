...
function renderTwoSideCook() {
  ...
  if (data.phase === "cook") {
    ...
    gauge = `<div class="skewer-flip-sequence" aria-label="꼬치 뒤집기 진행도">${Array.from({ length: 4 }, (_, index) => `<span class="skewer-flip-pair ${index < (data.flippedSkewers || 0) ? "done" : index === current ? "current" : ""}"><b>←</b><b>→</b></span>`).join("")}</div>
      <p class="cut-count" id="skewerFlipLabel">꼬치 ${(data.flippedSkewers || 0) + 1} / 4 · <strong>${data.flipStep === 1 ? "→" : "←"}</strong> 입력</p>`;
    ...
    gauge = `<p class="cut-count">${isSkewer ? "꼬치 4개" : "김치전"} 뒤집는 중…</p>`;
    control = twoSideKeysMarkup(view);
  }
