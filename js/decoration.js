...
/* 3-1. 메뉴 양옆 전등
   ...
   [가로 위치] 창 중심(x 945) 기준으로 495 씩 벌렸습니다.
   폭의 절반이 27 이라 등롱은 x 423~477 / 1413~1467 을 차지합니다.
   417/1473 → 357/1533 까지 벌렸다가 387/1503 으로 되돌렸고, 상단 HUD 우상단
   스탯 칸이 넓어지면서(왼끝 1503.3 → 1471.3) 427/1463 으로 당겼다가,
   그 스탯 칸에 오른쪽 등이 물리는 것을 없애려고 다시 23 씩 안쪽으로 옮긴 값입니다.
   ⚠️ 좌우는 늘 창 중심(945) 대칭이어야 합니다. 한쪽만 고치지 마세요.

   [안쪽 한계 = HUD] 등 두 개가 다 안 가리려면 창 중심에서 **412~499** 사이여야
   합니다. 바깥쪽 한계는 우상단 스탯 칸(x 1471.3~ · js/hud.css .hud-stats)이라
   495 + 27 = 1467 로 4 남습니다. 안쪽 한계는 메뉴 카드 줄(3장 · x 560.1~1272.1)
   인데 왼쪽 등 오른끝이 477 이라 아직 83 남습니다.
   ⚠️ 스탯 칸을 더 넓히면(=왼끝이 1467 아래로 오면) 오른쪽 등이 다시 물립니다.
      그때는 여기 495 를 (스탯칸 왼끝 - 945 - 27) 로 다시 잡으세요.

   [창틀에 살짝 걸치는 것은 허용] 495 는 "빈 벽"(창 왼쪽 307~463 · 창 오른쪽
   1435~1605)보다 13 씩 안쪽이라, 등롱 **밑동 바깥 모서리 약 10x5** 가 창틀
   (bg_window · x 464~1426 · y 130~ · depth 20)에 가려집니다. 등은 depth 18 이라
   창틀 뒤로 들어갑니다. 밑에 달린 구슬 장식(y 135~150)은 폭이 좁아 안 닿습니다.
   창틀까지 안 건드리려면 508 이상이어야 하는데, 그러면 위 HUD 한계(499)를
   넘어 오른쪽 등이 다시 스탯 칸에 물립니다 — 둘 다 만족하는 값은 없습니다. */
const DECORATION_LAMPS = [
  { key:"prop_menu_lamp_left",  cx: 450, top:0, h:150 },
  { key:"prop_menu_lamp_right", cx:1440, top:0, h:150 }
];
...
/* 3-6. 밤에만 켜지는 조리 연출 — 후라이팬 위 김치전
   ...
   [자리] 팬 아가리(테두리 은색 선)를 타원으로 실측하면
     중심 (1078.6, 419.5) · 가로 반지름 43 · 세로 반지름 20
     = x 1035.6~1121.7 · y 399.4~439.6
   입니다. 김치전은 이 타원 **안쪽에** 들어가야 팬에 담긴 것으로 보입니다.

   [폭 67] 아가리 폭이 86 이라 좌우로 9~10 씩 남습니다. 이 여백이
   팬 안쪽 벽이라, 이만큼 남아야 음식이 팬 "안에" 있는 것으로 읽힙니다.
   예전 값(81~88)은 여백이 2~3 밖에 안 남아 김치전이 팬 아가리를 덮은
   뚜껑처럼 보였습니다.

   [밑변 436] 아가리 앞변이 439.6 이라 앞쪽으로 테두리가 4 남습니다.
   김치전 세로 중심(422)이 아가리 중심(419.5)보다 2.5 아래라, 살짝
   내려앉아 담긴 느낌이 납니다. 밑변을 440 아래로 내리면 반죽이 팬
   앞 테두리를 타고 넘어갑니다. */
const DECORATION_NIGHT_PROPS = [
  { key:"food_kimchi_pancake_pan", cx:1078, bottom:436, h:28 }
];
...
/* 3-7. 밤에만 켜지는 조리 연기 (6프레임 스프라이트 시트)
   ...
   [발원점] 음식 윗면 바로 위입니다. 실측한 자리는
     후라이팬 김치전 윗면 (1078, 414)
     직화구이 꼬치 불꽃 위 (1218, 396) · (1262, 393)
     튀김기   기름 표면   (1388, 405)

   [직화구이만 두 줄기인 이유] 꼬치 다섯 개가 x 1195~1283 에 걸쳐 있어
   구이판이 가로로 넓습니다. 한가운데 한 줄기만 세우면 양옆 꼬치는
   안 익고 있는 것처럼 보입니다. 그래서 2번(1218)·4번(1262) 꼬치 위에
   하나씩 올려 판 전체가 달아오른 것으로 읽히게 했습니다.
   가운데(3번, 1240)를 비워야 두 줄기가 겹쳐 한 덩어리로 안 보입니다.

   [네 줄기를 다 어긋나게] 같은 애니메이션을 같은 속도로 돌리면 전부
   한 몸처럼 부풀었다 꺼집니다. 시작 프레임과 재생 속도를 따로 줘서
   서로 어긋나게 했습니다 — 특히 나란히 선 직화구이 두 줄기는 이게
   없으면 쌍둥이처럼 똑같이 움직입니다. */
...
const DECORATION_STEAM = {
  ...
  sources:[
    { cx:1078, cy:414, startFrame:0, timeScale:1.00 },   // 후라이팬
    { cx:1218, cy:396, startFrame:2, timeScale:0.85 },   // 직화구이 — 2번 꼬치
    { cx:1262, cy:393, startFrame:5, timeScale:1.08 },   // 직화구이 — 4번 꼬치
