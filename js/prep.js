...
/* 준비물이 놓이는 높이(y)에 대하여 — 바 상판을 실측한 값입니다.
   ...
   (583~588)가 시작되는 자리입니다.

   ⚠️ 진짜 아래 한계는 583 이 아니라 **손님 의자 윗변 576** 입니다.
      의자가 금색 테두리보다 7 위에서 시작합니다. 상판 그림만 보고 583 까지
      쓰면 그 아래 8 은 의자에 가려서 안 보입니다 — 진행 글자가 그랬습니다.
      (2026-08-10 실측: counter.chairs[].image.getBounds().top / 1.5 = 576.2.
       1920x1080 헤드리스로 게임을 띄워 Phaser 오브젝트에서 직접 잰 값입니다)

   그래서 세로로 **바구니 48 + 진행 글자 11** 을 의자 위에 다 넣어야 합니다.
     y 550 (예전값)  바구니 528~576 · 글자 576~587 — 글자가 통째로 의자 위
     y 543           글자 아래 끝 580 — 아직 의자에 걸침
     y 536 (지금값)  바구니 514~562 · 글자 562~573 — 의자 위로 3 남음
     y 530 이하      바구니 위쪽이 상판 뒷변(525)을 넘어 바닥으로 더 올라감

   바구니 윗변 514 는 주황 테두리(521~525)를 11 넘어섭니다. 상판 58 안에
   59 를 넣어야 해서 어차피 어딘가는 넘겨야 하는데, 위쪽은 넘겨도 "카운터
   안쪽에 놓인 것"으로 보이는 반면 아래쪽은 의자에 잘려 안 보입니다.
   ------------------------------------------------------------ */
const PREP_LAYOUT = {
  y: 536,          // 준비물이 놓이는 높이. 아래 한계는 손님 의자 윗변(576)입니다.
  ...
  // 간판은 논리 y 588 부터라 준비물(그림 514~562, 진행 글자 baseline 573)과
  ...
  /* 이름표는 준비물 위쪽. 아래에 두면 카운터 앞 의자에 가립니다.
     이 값은 명판 **글자 baseline** 이고, 판은 그보다 5+h/2 위부터입니다
     (draw-utils.js drawFixtureLabel). -32 일 때는 판 아랫변이 바구니 윗변에서
     2 밖에 안 떨어져 거의 붙어 보였습니다. -44 면 14 떨어져서 판이 바구니
     위에 확실히 떠 있습니다. 지금 값 기준 판은 논리 473~500 입니다.
     ⚠️ 더 올리려면 요리사가 서는 자리(iy 482)와 겹치는 것을 보세요 —
        판이 요리사 몸통을 가리기 시작합니다. */
  labelDy: -44,
  /* 진행 글자("0/2" · "준비 완료") baseline 을 바구니 아랫변에서 이만큼 아래.
     11 은 글자 높이(bold 11px)와 같아서, 글자 윗변이 바구니 아랫변에 딱
     닿습니다. 더 벌리면 글자가 의자(576)로 내려갑니다 — 지금도 아래 끝이
     573 이라 여유가 3 뿐입니다. */
  statusGap: 11,
...
function drawPrepObjects(){
  ...
  prepObjectLayout().forEach(item=>{
    ...
    const textY=item.y+(art?art.h/2+L.artDy+L.statusGap:18);
    if(done){
