...
const FX_GUIDE_GLOW = {
  /* 흰빛입니다.
     ⚠️ 따뜻한 노란빛(#ffd27a)으로 바꿨다가 되돌린 자리입니다. 색만 보면
        가게 등불과 어울리는데, 이 화면은 벽·바닥·집기·명판이 전부 나무색
        계열이라 노란 빛이 배경에 섞여 버립니다. 특히 명판 자체가 노란
        나무판이라 테두리 빛과 판의 경계가 안 보입니다. 무채색인 흰빛만
        이 화면에서 "배경이 아닌 것"으로 읽힙니다. 다시 노란 쪽으로
        가려면 명판을 어둡게 하는 등 판 색부터 손봐야 합니다. */
  color: "#ffffff",
  ...
  /* 한 번 얹은 세기(halo)를 passes 번 겹칩니다.
     ...
     알파를 낮 화면(제일 밝은 베이지 타일 벽)에서 잰 값입니다.
       .58 x3  132 / 100 /  53 / 10   벽 앞에서 거의 안 보입니다
       .72 x4  178 / 142 /  81 / 17
       .80 x4  193 / 152 /  82 / 11   ← 지금 값
     낮 화면이 기준입니다. 밤에는 배경이 어두워 같은 값이 더 잘 보입니다. */
  halo:   .8, haloPasses: 4,
