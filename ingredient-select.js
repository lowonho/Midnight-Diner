...
const FRIDGE_INGREDIENTS=Object.freeze({
  fishCake:{id:"fishCake",label:"어묵",icon:"🍢"},
  radish:{id:"radish",label:"무",icon:"🥕"},
  greenOnion:{id:"greenOnion",label:"대파",icon:"🌿"},
  anchovy:{id:"anchovy",label:"멸치",icon:"🐟"},
  tofu:{id:"tofu",label:"두부",icon:"⬜"},
  kimchi:{id:"kimchi",label:"김치",icon:"🥬"},
  pork:{id:"pork",label:"돼지고기",icon:"🥩"},
  flour:{id:"flour",label:"밀가루",icon:"🌾"},
  water:{id:"water",label:"물",icon:"💧"},
  chicken:{id:"chicken",label:"닭고기",icon:"🍗"},
  udon:{id:"udon",label:"우동면",icon:"🍜"},
  cabbage:{id:"cabbage",label:"양배추",icon:"🥬"},
  carrot:{id:"carrot",label:"당근",icon:"🥕"},
  yakisobaSauce:{id:"yakisobaSauce",label:"볶음우동 소스",icon:"🫙"},
  shrimp:{id:"shrimp",label:"새우",icon:"🍤"},
  egg:{id:"egg",label:"계란물",icon:"🥚"},
  breadcrumbs:{id:"breadcrumbs",label:"빵가루",icon:"🍞"},
  tteok:{id:"tteok",label:"떡",icon:"🍚"},
  gochujang:{id:"gochujang",label:"고추장",icon:"🌶️"},
  potato:{id:"potato",label:"감자",icon:"🥔"},
  oil:{id:"oil",label:"식용유",icon:"🫗"},
  starch:{id:"starch",label:"전분",icon:"🥣"},
  mushroom:{id:"mushroom",label:"버섯",icon:"🍄"},
  garlic:{id:"garlic",label:"마늘",icon:"🧄"},
  onion:{id:"onion",label:"양파",icon:"🧅"},
  cucumber:{id:"cucumber",label:"오이",icon:"🥒"}
});
...
function renderIngredientSelection(){
  ...
  dom.ingredientGrid.innerHTML=progress.board.map(id=>{
    ...
    return `<button class="ingredient-choice ${selected?"selected":""} ${hinted?"hint":""}" data-ingredient-id="${id}" type="button" ${selected?"disabled":""} aria-pressed="${selected}"><span aria-hidden="true">${item.icon}</span><strong>${item.label}</strong>${selected?"<i>담았어요</i>":""}</button>`;
  }).join("");
  ...
  dom.ingredientBasket.innerHTML=pickedRequired.length?pickedRequired.map(id=>{
    const item=ingredientInfo(id);return `<span><i>${item.icon}</i>${item.label}</span>`;
