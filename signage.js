...
/* ------------------------------------------------------------
   ...
   [1:1 로 그리는 이유]
   프레임 캔버스는 화면 크기와 무관하게 항상 1920x1080 입니다. 그래서 그림을
   화면에 그려지는 픽셀 수(152x110 x1.5 = 228x165)로 미리 뽑아 두고 여기서는
   확대도 축소도 하지 않습니다. 리샘플링을 아예 거치지 않는 것이 금테와 글자를
   가장 또렷하게 유지하는 방법입니다.
   (마스터는 assets/UI/Common/ui_shop_status_*_4x.png 이고, 228x165 WebP 는
    tools/build-ui-common-webp.js 산출물입니다)

   ⚠️ [x 가 1104 인 이유] 1:1 로 그리려면 VIEW 좌표가 **정수**여야 합니다.
      1105 x1.5 = 1657.5 로 반픽셀에 걸쳐 있었습니다. 예전에는 프레임 캔버스가
      최근접 이웃이라 반올림돼 선명했지만, 2026-08-08 에 보간을 켜면서
      (stage.js createStageFrameTexture) 반픽셀이면 좌우 두 픽셀에 반씩 섞여
      간판만 가로로 흐려지게 됐습니다. 1104 x1.5 = 1656 이라 딱 떨어집니다.
      x 를 다시 옮길 때는 **짝수**로 두세요. y(588)·w(152)·h(110) 는 이미
      x1.5 가 정수입니다.
   ------------------------------------------------------------ */

const OPEN_SIGN = {
  x:1104, y:588, w:152, h:110,
