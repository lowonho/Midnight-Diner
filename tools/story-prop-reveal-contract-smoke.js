...
/* 소품 띄우기(story-prop-reveal.js)의 계약 검사.
   지키려는 것은 넷입니다.
...
const propSource = fs.readFileSync(path.join(root, "story-prop-reveal.js"), "utf8");
...
assert(indexSource.indexOf('src="story-prop-reveal.js"') < indexSource.indexOf('src="story.js"'),
  "story-prop-reveal.js 는 story.js 보다 먼저 로드되어야 합니다.");
...
/* story-data.js 가 부르는 소품 이름이 실제로 등록돼 있는지.
...
for (const prop of usedProps) {
  assert(declaredProps.has(prop), `story-data.js 가 없는 소품을 부릅니다: ${prop}`);
}
