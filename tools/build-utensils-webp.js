...
/* ============================================================
   ...
   [지금은 시트 에셋이 하나도 없습니다] 2026-08-09 에 밤 조리 연출 시트
   4종을 걷어내면서(js/kitchen.js §1-1) 이 갈래를 타는 파일이 없어졌습니다.
   지우지 않고 둔 이유는, 시트를 다시 넣을 때 위 MAX_EDGE 가 칸 폭을
   소수로 만들어 조용히 망가뜨리는 걸 막아 주기 때문입니다.

   시트를 만들어 그리는 도구는 이미지 생성 특성상 칸마다 그림이 가로로
   조금씩 밀리는 일이 흔합니다. 실제로 걷어낸 시트 4종이 그랬습니다 —
   바닥선은 6칸 모두 완벽히 같은데(0px) 가로로만 단조롭게 19~22px 흘러가서,
   재생하면 집기가 왼쪽으로 미끄러지다가 루프가 돌 때 튕겨 돌아왔습니다.

   그래서 빌드가 칸마다 밀린 양을 재서 **정수 픽셀로 되돌립니다.** 정수
   이동이라 다시 샘플링하지 않고, 따라서 화질 손실이 없습니다. 밀림이 없는
   시트를 넣으면 측정값이 0 이 되어 보정도 저절로 없어집니다.
...
/* 밀림을 되돌린 시트를 만들어 돌려줍니다 (원본 해상도 PNG 버퍼).
   ...
   js/kitchen.js 가 배치 기준으로 쓰는 값이라 바뀌면 알아야 합니다. */
