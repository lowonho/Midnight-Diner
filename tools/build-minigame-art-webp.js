...
const FILES = [
  ...
  /* 굽는 김치전 5장. 익힘 단계마다 한 장이고 전부 팬 안 같은 자리에 겹칩니다
     ...
     (팬 710 x 308.4 에서 테두리 9 를 뺀 692 x 290.4 의 `inset: 14% 27% 16% 7%`
      — ⚠️ % 는 테두리 **안쪽** 상자 기준이라 팬 크기에서 바로 곱하면 안 됩니다).
     그림 비율(2.058)이 상자(2.246)보다 좁아 **세로가 먼저 막습니다** → 418.4 x 203.3.
     그 2배율이 837 x 407 입니다.

     ⚠️ **다섯 장을 한 크기로 묶는 것이 중요합니다.** 마스터가 1424x690 / 1426x693 로
        raw 한 장만 캔버스가 다릅니다(비율 차 0.3%). 여기서 한 크기로 뽑아야
        익힘 단계가 넘어갈 때 김치전이 들썩이지 않습니다.
        (E8 불리기 볼 11장 · E4 냄비 24장과 같은 처리입니다.)

     [q88] 다섯 장이라 용량이 곱해집니다. 다만 반죽 표면이 잘게 부푼 결이라
     E4 냄비(82) · E8 볼(85) 만큼 내리지는 않았습니다 — 88 에서 장당 130~150KB 이고,
     --verify 의 RGB 평균 오차가 6~7 로 같은 결의 E8 김치 그림(6.79)과 같은 수준입니다. */
  ...
  /* ---- E5 닭꼬치 굽기 : 숯불 화로 5장 -------------------------------
     ...
     [크기] 화면 684 x 409.8 의 2배율입니다. 이 값은 굽기 칸(824.2 x 457)에서
     **꼬치 손잡이가 화로 아래로 나오는 36 을 뺀 키**가 상한이라 정해집니다
       409.8 + 36 = 445.8  ≤  457      (여유 11)
     가로는 그 키에 그림 비율(1.6693)을 곱한 값이라 칸 824.2 중 684 만 씁니다 —
     좌우 70 씩 나무 상판이 보이는 것이 맞습니다. 칸 가로를 꽉 채우려면 키가
     494 가 되어 손잡이가 .ts-board 의 overflow:hidden 에 잘립니다.
     ⚠️ 화면 자리를 바꾸면 여기 크기와 css 의 `.ts-board .two-side-pan.skewer-cook` 을
        **함께** 고쳐야 합니다.

     ⚠️ **다섯 장을 한 크기로 묶는 것이 중요합니다.** 납품본이 2761~2768 x 1656~1658 로
        장마다 캔버스가 조금씩 다릅니다(비율 차 최대 0.3%). 그림이 캔버스를 거의 꽉
        채우고 있어 여백을 덧댈 수가 없으므로, E4 냄비 24장처럼 **여기서 한 크기로
        뽑아** 맞춥니다. 안 그러면 숯이 달아오를 때마다 화로가 1px 씩 들썩입니다.
        checkAspect 한계(2%)의 1/7 이라 찌그러짐은 눈에 안 보입니다.
     [q85] 다섯 장이라 용량이 그대로 곱해집니다. 숯 표면이 잘게 갈라진 결이지만
     대부분 어두워서 85 에서도 눈에 띄는 손실이 없습니다 (E8 볼 11장과 같은 판단입니다). */
  ...["01","02","03","04","05"].map(no=>({
    file:`E5/yakitori/fix_charcoal_grill_fire_${no}.png`, size:[1368,820], quality:85,
    css:".ts-board .two-side-pan.skewer-cook 684x409.8 (5장 한 크기)" })),
  /* ---- E6 튀기기 (새우튀김 · 감자튀김) -----------------------------
     [가스 버너 상판] 가운데 플레이 칸(824.2x613.2)을 통째로 덮는 바닥입니다.
     E3·E4 의 옆에서 본 화구(.mg-burner)와 달리 **위에서 본 한 장**이라
     불꽃 애니메이션이 없습니다 — 액자까지 그림에 그려져 있습니다.
     ⚠️ stretch 입니다. 마스터 비율 1.4414 가 칸(1.3441)보다 넓어 6.8% 눌립니다.
        눌리는 것은 대부분 화구 링인데 **그 위에 냄비(619.8)가 덮여** 안 보이고,
        밖으로 나온 것은 방사형 살대라 눌린 티가 안 납니다.
        (칸에 맞춰 여기서 미리 눌러 두면 CSS 는 object-fit:cover 한 줄로 끝납니다) */
  { file:"E6/fix_gas_burner_top_3297x2453.png", size:[1648,1226], stretch:true, css:".fry-work-area 824.2x613.2" },
  /* [기름 냄비 2장] 냄비 한 장과 **그 안의 기름만 따로 뽑은 한 장**입니다.
     캔버스가 1081x983 로 같아서 겹치면 딱 맞습니다 — 화면에서도 같은 자리·같은
     크기로 깔고(css 의 `.fry-pot-asset, .fry-oil-veil-asset`), 기름 쪽만 튀김
     **위에** 반투명으로 한 번 더 덮습니다. 그게 "기름에 잠겼다" 의 전부입니다.

     **화면 상자(.fry-pot)는 냄비 전체가 아니라 기름 타원입니다.**
     조각 자리(--x/--y)가 이 상자 기준 0~100 이라, 상자가 기름과 같아야
     engine-e6 의 FRY_POT_RADIUS 가 눈에 보이는 기름 안쪽과 맞습니다.
       기름 타원  기름 레이어에서 실측 — x 12.86% w 74.38% · y 7.02% h 85.15%
                  (= 804x837. 냄비 그림에서 색으로 재면 가장자리 어두운 띠를
                   놓쳐 더 작게 나옵니다. **기름 레이어 쪽이 기준입니다.**)
       화면 상자  461 x 479.9  →  냄비 그림 619.8 x 563.6

     ⚠️ **둘 다 원본 배율 그대로입니다(size:null).** 2배율은 1240x1127 인데 마스터가
        1081x983 (1.74배) 뿐입니다. 늘리면 없던 화소를 지어내는 셈이라 그대로 둡니다
        (ui_play_tray_wood · E3 화구와 같은 판단입니다).
     ⚠️ 기름막만 **기름 원 자리 [139, 69, 804x837] 로 잘라 냅니다.** 그래야 화면
        상자(.fry-pot)와 그림이 1:1 이 되어, 냄비 그림처럼 밖으로 넘기는 계산 없이
        CSS 쪽이 inset:0 한 줄로 끝납니다. 잘리는 것은 원 둘레의 안티에일리어싱
        2px 뿐입니다(화면에서 1.1px).
        새 납품본을 받으면 **이 자리부터 다시 재세요** — 여기가 어긋나면 기름은
        멀쩡한데 튀김 놓이는 자리만 밀립니다. 재는 법은 알파가 꽉 찬(>200) 화소의
        가로 최장 구간과 그 구간이 있는 줄의 범위입니다. */
  { file:"E6/fix_fry_pot_oil_smooth.png", size:null, css:".fry-pot 461x479.9 안의 기름 타원 기준 (냄비 전체 619.8x563.6 · 원본 배율 유지)" },
  { file:"E6/fix_fry_oil_smooth_01.png",  size:null, crop:[139,69,804,837],
    css:".fry-pot 461x479.9 를 그대로 덮음 (튀김 위에 반투명으로 얹는 기름막)" },
  /* [기름빼기 트레이] 오른쪽 '완성' 칸입니다. 칸 폭 225.6 이 먼저 막혀
     225.6x356.3 으로 그려집니다(.fry-rack 이 이 비율을 그대로 답니다). */
  { file:"E6/prop_fry_drain_tray_empty.png", size:[451,713], css:".fry-rack 225.6x356.3" },
  /* [튀김 3단계 x 2종] 설익음 → 잘 익음 → 탐.
     ⚠️ **설익음 장은 왼쪽 재료 칸에도 그대로 씁니다** — 아직 안 튀긴 재료와
        갓 넣은 튀김이 같은 그림입니다. 재료 칸(약 108x101.6)이 훨씬 작아서
        아래 냄비 기준 크기 하나로 둘 다 덮습니다.
     ⚠️ **세 장을 한 크기로 묶는 것이 중요합니다.** 냄비 안에서 세 장이 한 자리에
        겹쳐 있다가 익힘에 따라 서로 겹쳐 넘어가므로(css 의 .fry-art-asset[data-art]),
        크기가 다르면 단계가 바뀔 때마다 튀김이 들썩입니다. 새우 마스터가
        1003x942 / 1200x1127 / 1199x1126 으로 다른데 비율은 셋 다 1.0648 로 같습니다.
     냄비 안 크기(.fry-piece --w) 의 2배율입니다 — 새우 142.1 · 감자 125.4.
     ⚠️ 무손실입니다. 튀김옷 알갱이와 감자 가닥이 전부 또렷한 픽셀 테두리라
        q90 에서 RGB 평균 오차가 4~5 까지 올라갔습니다(다른 그림은 1~2). 그 링잉이
        알갱이 하나하나 둘레에 그대로 얹혀 튀김이 지저분해 보입니다.
        여섯 장뿐이고 화면의 주인공이라 용량(장당 60~70KB)을 씁니다. */
  ...["undercooked","perfect","burnt80"].map(stage=>(
    { file:`E6/food_shrimp_tempura_${stage}.png`, size:[284,267], lossless:true, css:".fry-piece 142.1x133.5 (새우 3단계 한 크기)" })),
  ...["undercooked","perfect","burnt80"].map(stage=>(
    { file:`E6/food_fries_${stage}.png`, size:[251,225], lossless:true, css:".fry-piece 125.4x112.6 (감자 3단계 한 크기)" })),
  /* [기름 방울 3장] 집게로 건져 올린 튀김에서 떨어지는 기름입니다. 셋이 동시에
     서로 다른 자리에서 떨어지므로 **한 크기로 묶지 않습니다** — 길이가 다른 것이
     곧 방울이 떨어진 거리 차이입니다. 화면에서는 가로만 12 로 맞춥니다.
     ⚠️ 무손실입니다. 가느다란 획과 반짝임이라 q90 링잉이 바로 드러납니다
        (E2 채 9종 · E8 물방울과 같은 판단입니다). 원본이 15KB 라 부담이 없습니다. */
  { file:"E6/fx_fry_oil_drop_01.png", size:[24,172], lossless:true, css:".fry-oil-drip 12x85.9" },
  { file:"E6/fx_fry_oil_drop_02.png", size:[24,137], lossless:true, css:".fry-oil-drip 12x68.6" },
  { file:"E6/fx_fry_oil_drop_03.png", size:[24,233], lossless:true, css:".fry-oil-drip 12x116.7" },
  /* [집게 커서 3장] 이 화면만 쓰는 마우스 포인터입니다. **CSS cursor 가 아니라
     DOM 으로 그립니다**(engine-e6 의 mountFryCursor) — 집은 튀김을 아래팔과 윗팔
     사이에 끼워야 해서 한 장짜리 커서로는 안 되고, 그 덕에 크롬의 128px 상한도
     받지 않습니다. 그래서 화면 크기(.fry-cursor 210x235.8)의 2배율입니다.
     ⚠️ `ui_cursor_catpaw_tongs_closed.png`(닫힌 집게 한 장)은 **뽑지 않습니다.**
        아래팔+윗팔을 겹친 것과 픽셀까지 같아서(실측 최대 오차 0) 두 장으로 냅니다.
        안 쓰는 WebP 를 만들지 않기 위해서입니다. */
  { file:"E6/ui_cursor_catpaw_tongs_open.png",          size:[420,472], css:".fry-cursor 210x235.8 (안 누른 상태)" },
  { file:"E6/ui_cursor_catpaw_tongs_closed_bottom.png", size:[420,472], css:".fry-cursor 210x235.8 (누른 상태 아래팔)" },
  { file:"E6/ui_cursor_catpaw_tongs_closed_top.png",    size:[420,472], css:".fry-cursor 210x235.8 (누른 상태 윗팔)" },
  /* ---- E2 채썰기 -------------------------------------------------
  ...
  /* [넓은 트레이 3장] 지금 화면이 실제로 쓰는 그릇입니다. 위 볼 3장은 세 그릇을
     한꺼번에 늘어놓던 예전 화면(.fp-vessel)용이라 지금은 안 쓰입니다 — 파일과
     이 표의 줄은 남겨 둡니다(되돌릴 때 다시 필요합니다).

     새우를 **좌우로 굴리는** 놀이라 위에서 내려다본 둥근 볼보다 옆으로 긴
     트레이가 맞습니다. 셋 다 한 규격(1584x562)입니다.
     ⚠️ 납품본은 1624x566 이었고 **좌우에 반투명 흰 후광이 20px 씩** 있었습니다.
     ...
     ⚠️ 자리(.fp-roll-vessel 789x330)는 **그림보다 세로로 깁니다** — 새우가
        ...
        그래서 여기 크기는 자리 비율이 아니라 **원본 비율 그대로** 둡니다. */
  ...["flour","egg","breadcrumb"].map(name=>({
    file:`E2/shrimp/food_shrimp_tempura_${name}_tray_wide_75deg.png`,
    size:[1584,562], css:".fp-roll-vessel 789x330 (세로로 늘려 담음)" })),
