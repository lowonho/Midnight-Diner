...
/* ============================================================
   ...
   리사이즈는 하지 않습니다. 배율이 들어가면 js/food-props.js 의 규격 계산과
   ...
   ⚠️ 2026-08-08 낮 준비물 그림 8장이 **4배로 업스케일**되어 다시 들어왔습니다
      (전부 1056x608 = 264x152 x4). 자도 4배(js/food-props.js FOOD_PREP_SIZE)라
...
const JOBS = [
  ...
  /* 낮 준비물 그림은 4배 원화라 자도 4배입니다(js/food-props.js FOOD_PREP_SIZE).
     trim 을 켜는 이유: 원화가 여백까지 꽉 찬 1056x608 로 와서, 그대로 두면
     그림자·완료 도장·진행 숫자가 바구니가 아니라 **빈 캔버스 가장자리**를
     기준으로 붙습니다(js/prep.js drawPrepArt 는 그려진 크기를 받아서 씁니다).
