...
/* 주인공 대화 원화(모션) 계약 검사
   ...
   js/story-data.js 의 대사에 적힌 motion 과 실제 파일·CSS 를 맞춰 봅니다.
   여기서 막고 싶은 사고는 다섯입니다.
     ...
     4. --portrait-art 에 상대경로 — 커스텀 속성 안의 url() 은 그 값을 쓰는
        ...
        404 가 납니다. js/story.js 는 반드시 문서 기준 절대 URL 로 넘겨야 합니다.
...
const dataSource = fs.readFileSync(path.join(root, "js/story-data.js"), "utf8");
...
/* ---- 1. js/story.js 가 쓰는 모션 표 읽기 ---- */
const tableBlock = storySource.match(/const STORY_PROTAGONIST_MOTIONS=Object\.freeze\(\{([\s\S]*?)\}\);/);
if(!tableBlock) fail("js/story.js 에서 STORY_PROTAGONIST_MOTIONS 표를 못 찾았습니다.");
...
if(!artBlock) fail("js/story.js 에서 STORY_PORTRAIT_ART 표를 못 찾았습니다.");
...
for(const [, key] of dataSource.matchAll(/motion:\s*"(\w+)"/g)){
  if(!motions.has(key)) fail(`js/story-data.js 에 없는 모션 "${key}" 가 적혀 있습니다. 쓸 수 있는 값: ${[...motions.keys()].join(", ")}`);
  used.add(key);
}
if(!used.size) fail("js/story-data.js 에 motion 이 하나도 없습니다.");
...
if(!storySource.includes("new URL(source,document.baseURI)")){
  fail("js/story.js 가 --portrait-art 를 문서 기준 절대 URL 로 만들지 않습니다.\n"
...
// 상자 치수는 이제 js/story.js 가 인라인으로 넣습니다. CSS 에는 안전값만 남습니다.
if(!/--art-height/.test(storySource)){
  fail("js/story.js 가 --art-height 를 인물별로 넣지 않습니다.\n"
