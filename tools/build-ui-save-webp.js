...
/* ============================================================
   ...
   즉 "슬롯 한 줄의 폭"만 정하면 나머지 다섯 장의 크기가 전부 따라옵니다.
   css/save-slots.css 는 --sv-panel-h(판 세로)와 --sv-slot-inset(줄을 금테에서
   더 들여 놓는 값) 둘만 두고 나머지를 계산합니다.
   여기 표도 그 계산 결과(2026-08-07 기준 --sv-panel-h: 860upx)의 2배입니다.

     판 948x860 · 슬롯 한 줄 폭 840 · 카드 840x142 / 740x142 ·
     삭제 89x142 · 닫기 48x48 · 빈 칸 안내문 128x48

   ⚠️ css/save-slots.css 의 --sv-panel-h 를 키우거나 --sv-slot-inset 을 줄이면
      아래 size 도 같이 키워야 합니다. 안 그러면 확대되면서 뭉갭니다.
      (checkAspect 가 경고합니다)
   ...
   반대로 닫기(48upx 원)와 빈 칸 안내문(128x48)은 무손실이어도 합쳐 40KB 가
...
const FILES = [
  ...
  /* ── 슬롯 카드 2종 ─────────────────────────────────────────
     ...
     둘 다 세로는 같으므로 한 줄 높이는 어느 쪽이든 142 로 같습니다. */
  { file:"ui_save_card_with_delete.png",    size:[1480,285], why:"삭제 버튼 있는 줄의 카드 740x142 x2" },
  { file:"ui_save_card_without_delete.png", size:[1680,285], why:"빈 슬롯 줄의 카드 840x142 x2 (슬롯 한 줄 폭 전체)" },

  /* ── 카드 옆 삭제 버튼 ─────────────────────────────────────
     카드와 세로가 같아서 줄 높이에 그대로 맞물립니다. */
  { file:"ui_save_delete_button.png", size:[178,285], why:"삭제 버튼 89x142 x2" },
  ...
  /* 안내문만 원본이 곧 2배입니다 (128x48 x2 = 256x96). 더 키우면 확대가
     되므로, 이보다 크게 쓰고 싶으면 원화를 다시 받아야 합니다. */
  { file:"ui_save_empty_notice.png", size:[256,96],  lossless:true, why:"빈 칸 안내문 128x48 x2 = 원본 크기" }
];
