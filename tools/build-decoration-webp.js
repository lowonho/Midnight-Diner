...
/* ============================================================
   ...
   대상: assets/bg/decoration/  (서비스 벨 · 메뉴 양옆 전등 ·
         정면 벽 장식 · 좌측벽 화분/게시판)
   배치값은 js/decoration.js 한 곳에 있습니다.
   ...
   캔버스 크기는 건드리지 않습니다. js/decoration.js 의 배치표가
...
/* 파일별 품질 예외. 옆의 배율은 "게임에서 그리는 크기 ÷ 원본 크기" 입니다.
   (배치값은 js/decoration.js §3-5 ~ §3-7 에 있습니다)
...
/* 원본 PNG 와 변환된 WebP 를 픽셀 단위로 비교합니다.
   ...
   js/decoration.js 의 trim 값을 다시 재세요. */
...
/* js/decoration.js 의 trim 표가 아직 맞는지 확인합니다.
   에셋을 다시 받아 여백 두께가 바뀌면 배치가 통째로 밀리는데,
   화면을 보기 전에는 눈치채기 어려워서 여기서 같이 찍어 줍니다. */
async function printTrim(){
  console.log("\n불투명 영역 실측 (js/decoration.js DECORATION_ART.trim 과 같아야 합니다)");
  for(const name of files){
