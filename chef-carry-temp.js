...
/* ------------------------------------------------------------
   ...
   아래 값은 에셋 묶음에 같이 온 carry_anchor_table.md 를 옮긴 것입니다.
   그 표는 2× 셀(384×640) 좌상단 기준이라 절반으로 줄이고, 발바닥 중앙
   (96, 320) 을 빼서 오프셋으로 바꿨습니다. idle_carry 0번 프레임 기준.
     down (192,326) → (96,163) → ( 0,   -157)
     up   (192,350) → (96,175) → ( 0,   -145)
     side (141,325) → (70.5,162.5) → (-25.5, -157.5)
   up 이 유독 낮은 건 뒷모습이라 손이 몸에 가려 더 아래에서 보이기 때문입니다.

   표에는 프레임별 값도 있지만 편차가 ±2px 라서 방향당 하나로 씁니다.
   눈으로 보고 어긋나면 IJKL 로 맞춘 뒤 O 키로 찍어서 여기에 덮어쓰세요.
   ------------------------------------------------------------ */
