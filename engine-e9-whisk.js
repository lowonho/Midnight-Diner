...
const WHISK_INNER_LIMIT=.12;   // 볼 한가운데를 문지르는 것은 세지 않습니다 (작업 영역 지름 대비)
const WHISK_OUTER_LIMIT=.52;   // 볼 바깥을 도는 것도 세지 않습니다
const WHISK_GAIN=6;            // 1 라디안 돌 때 오르는 진행도

function setupWhiskBatter(){
  const m=setDayPrepData({mode:"whisk",progress:0,spin:0,pointerActive:false,lastAngle:null});
  if(!m)return;
...
function moveWhiskPointer(event){
  ...
  const rect=work.getBoundingClientRect(),size=Math.min(rect.width,rect.height);
  ...
  if(m.data.lastAngle!=null&&radius>size*WHISK_INNER_LIMIT&&radius<size*WHISK_OUTER_LIMIT){
    const delta=Math.atan2(Math.sin(angle-m.data.lastAngle),Math.cos(angle-m.data.lastAngle));
    if(Math.abs(delta)<1.2){
      m.data.progress=clamp(m.data.progress+Math.abs(delta)*WHISK_GAIN,0,100);
  ...
  if(progress>=100){
    ...
    finishDayPrepTask("mixKimchiBatter","김치전 반죽 완성");
  }
}
